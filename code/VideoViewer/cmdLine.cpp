    ...
    while (iArg < argc) {
        if (!Parameter::IsArg(argv[iArg])) {
            ...
            if (!processed) {
                ...
                cout << "Unrecognized parameter: " << argv[iArg] << std::endl << std::endl;
                DoHelp(*argv);
                return false;
            }
        }
...
void CmdLine::Dump(std::ostream& outstream) const
{
    for (size_t iParm = 0; iParm < m_Params.size(); ++iParm) {
...
bool CmdLine::CheckUnique() const
{
    ...
    for (size_t iParm = 1; iParm < m_Params.size(); ++iParm) {
        ...
        if (prev >= 0) {
            cout << "Parameter obscured by earlier parameter:" << std::endl;
            m_Params[iParm]->Print(std::cout);
            cout << "will be ignored, earlier paramter:" << std::endl;
            m_Params[prev]->Print(std::cout);
            cout << "will be used." << std::endl;
            errors = true;
        }
    }
    return errors;
